...
class GCNTox21(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features,hidden_dim=256, num_layers=3, dropout=0.2):
        ...
        for i in range(num_layers):
            ...
            self.convs.append(EdgeConv(mlp, aggr='mean'))
            self.bns.append(torch.nn.BatchNorm1d(hidden_dim))
        
        # Fully connected output layer
        self.fc = torch.nn.Linear(hidden_dim, 12)  # Update to 12 tasks
...
log_dir = f'runs/final runs/GCN_node_tox21_{current_time}'
...
if __name__ == '__main__':
    ...
    with open('grid search/results/grid_search_GCN_node_results.json', 'r') as f:
    ...
    plt.legend()
    plt.savefig(f'precision_rcall_curv/GCN_node_tox21.png')
    plt.show()
